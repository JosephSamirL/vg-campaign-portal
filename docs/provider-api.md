...
| d | Field names: id / type / timestamp / recipient | Envelope `{"batch_id","events":[…],"next_cursor","has_more"}`; event `{"event_id":"evt-batch_5a-00000","recipient_id":"PROBE-2026-09-15-6ab4dc49","brand_code":"account","type":"delivered","occurred_at":"2026-09-15T18:18:42Z"}` (e1). Recipient key is **`recipient_id`** (value = the `external_id` we posted), not `external_id`; timestamp is **`occurred_at`** (ISO-8601, `Z`, second precision); `brand_code` is `"account"` on our own events — not the `brand` label we posted (`probe`) — and `"KAROO"` on forged ones | `event_id`, `type`, `occurred_at`, `recipient_id` | 6.2 coalesce order: id `event_id ?? id`; recipient `recipient_id ?? external_id ?? id ?? contact_id ?? recipient_id ?? email`; time `occurred_at ?? timestamp` (present on every event seen — the "absent timestamp" path stays a fallback). Tenancy comes from the send's snapshot only: **never** trust `brand_code` |
| d2 | Types seen; order/duplicates seen | Types: `delivered`, `opened` (no `bounced`/`unsubscribed` in 13 min). Mess, all within ~3 min of the POST (e5, t1, t1b, t2b): (1) exact duplicates — the same `event_id` twice in one page (`…-00000` ×2, `…-00001` ×2); (2) out of order — `delivered` (`occurred_at` 18:19:00) re-appended **after** `opened` (18:22:10); (3) **forged** events `evt-<batch>-forged` for a recipient that was **not** in the batch — `recipient_id` `CT-078480` / `CT-021214` are real seed contacts (locally: KILELE) labelled `brand_code:"KAROO"`; (4) **future** `occurred_at` — `opened` at 18:22:10Z served at 18:21:28Z, forged at 18:21:53Z served at 18:19:36Z; (5) accumulation: 1 event at 18:18:20 → 3 at 18:19:36, then stable through 18:31 (t3) | as observed | Never relaxes FR-24. 6.2: dedupe on `(batch_id, event_id)`; drop (and count) any event whose `recipient_id` is not in the send's `send_recipients`; keep `occurred_at` verbatim in `raw`/`occurred_at` but let precedence, not time, decide the visible state; `opened` may precede `delivered` |
| e | 401 / 404 / 429 shapes, `Retry-After`? | **401** `{"error":"unauthorized","message":"Provide your API key as 'Authorization: Bearer <key>' or 'X-API-Key: <key>'."}` — same body for a wrong bearer (a1) and no header (a2). **404** `{"error":"not_found","message":"unknown batch_id"}` (e6). **429**: not observed (r1: 40 back-to-back GETs → 28×`200`, 12×`503`, 0×`429`; ~70 GETs total). **503** `{"error":"service_unavailable","message":"reports temporarily unavailable","retry_after":N}` **with** `Retry-After: N` header (seconds; a 12-request 503 burst counted down 5,4,4,3,3,3,2,2,2,1,1,1; 18 and 16 seen on isolated hits) — ~30 % of GETs, on both auth styles (e5-first-try, x1-first-try, r1). Error bodies are always `{"error":"<snake_case>","message":"<text>"}` | JSON shapes as quoted | 6.3: 401 → `auth_error` (stop the run); 404 → `provider_error` for that batch; 429 **and 503** → honour `Retry-After` (header, else body `retry_after`, else 5 s), retry within the run budget, and a run that ends on a 503 is `deferred`, not a failure of the batch. Provider 503s on POST were not seen (3/3 `200`) — the sweep + same key cover that path |
...
3. **Cursor rules for 6.3** (replace D-8's fallback where they differ):
   ...
   - A `503`/`429` honours `Retry-After` (header → body `retry_after` → 5 s); a run that runs out of budget while waiting records `deferred` for that batch, not an error. `401` → `auth_error`; `404` → `provider_error`.
